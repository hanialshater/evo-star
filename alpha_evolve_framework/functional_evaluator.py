...
def _execute_in_process(eval_fn: Callable, code: str, config: Dict[str, Any], result_queue: mp.Queue):
    """
    ...
    try:
        ...
        scores, details = eval_fn(code, config)
...
class FunctionalEvaluator(BaseEvaluator):
    ...
    def evaluate(self,
        ...
        process = mp.Process(
            ...
            args=(self._eval_fn, full_code_to_evaluate, runtime_config, result_queue)
        )

        logger.info(f"Starting evaluation for {program_id} in a subprocess with a {timeout_seconds}s timeout.")
        process.start()
        process.join(timeout=timeout_seconds)

        if process.is_alive():
            # Process is still running, meaning it timed out.
            logger.warning(f"Evaluation for {program_id} timed out after {timeout_seconds} seconds. Terminating process.")
            process.terminate() # Send SIGTERM
            process.join()      # Wait for termination to complete

            scores = {'main_score': -2000.0} # Special score for timeouts
            details = {'error_message': f'Evaluation timed out after {timeout_seconds} seconds.'}
