...
from .evaluator_abc import BaseEvaluator # Relative import
...
class BaseOptimizer(abc.ABC):
    def __init__(self,
        ...
        print(f"BaseOptimizer (Island {self.island_id}) initialized for problem: {self.problem_name}")

    @abc.abstractmethod
    def initialize_population(self): pass

    @abc.abstractmethod
    def ask(self) -> List[Dict[str, Any]]: pass

    @abc.abstractmethod
    def tell(self, evaluated_programs: List[Program]): pass

    @abc.abstractmethod
    def get_best_solution(self) -> Optional[Program]: pass

    @abc.abstractmethod
    def get_emigrants(self, num_emigrants: int) -> List[Program]: pass

    @abc.abstractmethod
    def receive_immigrants(self, immigrants: List[Program]): pass
    ...
    def should_terminate(self) -> bool:
        ...
        if self.run_config.use_island_model:
            ...
            return self.current_generation >= self.run_config.island_generations_per_epoch
        else:
