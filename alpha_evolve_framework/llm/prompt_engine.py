...
class PromptEngine:
    def __init__(
        self, task_description: str, problem_specific_instructions: Optional[str] = None
    ):
    ...
    def build_evolution_prompt(
        ...
        if not target_evolve_block:
            raise ValueError(f"Target block '{target_block_name}' not found.")
        ...
        if self.problem_specific_instructions:
            parts.extend(
                [
                    ...
                    self.problem_specific_instructions,
                ]
            )
        ...
        if parent_program and parent_program.judge_feedback:
            ...
            critique = parent_program.judge_feedback.get(
                "critique", "No critique provided."
            )
            suggestions = parent_program.judge_feedback.get(
            ...
            parts.append(
                "# IMPORTANT: You MUST address the judge's feedback in your new version."
            )
        ...
        if refinement_feedback:
            ...
            parts.append(
                "Do not feel bound by the existing implementation if you have a better fundamental approach."
            )
            parts.append(
        ...
        if context_programs:
            if not allow_full_rewrites:
                ...
                for i, ctx_prog in enumerate(context_programs):
                    parts.extend(
                        [
                            f"## Context Example {i+1} (Score: {ctx_prog.scores.get('main_score', 'N/A'):.4f}):",
                            "```python",
                            ctx_prog.code_str.strip(),
                            "```",
                        ]
                    )
        ...
        if output_format == "diff":
            parts.extend(
                [
                    "\nPlease provide your modification using the following diff format.",
            ...
            parts.extend(
                [
                    ...
                    "Your response should be ONLY the Python code, enclosed in a ```python ... ``` block.",
                ]
            )
