...
class LLMBlockEvolver(BaseOptimizer):
    def __init__(self, initial_codebase: Codebase, llm_manager: LLMManager, program_database: BaseProgramDatabase, prompt_engine: PromptEngine, evaluator: BaseEvaluator, run_config: RunConfiguration, feature_definitions: Optional[List[Dict[str, Any]]] = None, feature_extractor_fn: Optional[Callable] = None, problem_specific_feature_configs: Optional[Dict[str, Any]] = None, island_id: Optional[int] = None):
        super().__init__(problem_name=evaluator.problem_name, run_config=run_config, evaluator=evaluator, island_id=island_id)
    ...
    def initialize_population(self, initial_population: Optional[List[Program]] = None):
        ...
        if initial_population:
            logger.info(f"Island {self.island_id} seeding population from previous stage.")
            self.tell(initial_population)
        else:
            logger.info(f"Island {self.island_id} initializing new population with seed program.")
    ...
    def ask(self) -> List[Dict[str, Any]]:
        ...
        for _ in range(self.run_config.candidates_per_ask):
            parent_program = self.program_database.select_parent_program() or self.overall_best_solution
            if not parent_program:
                logger.warning(f"Island {self.island_id}: Could not select a parent program to evolve.")
                continue

            target_block_name = parent_program.block_name or self.working_codebase.get_block_names()[0]
            context_programs = self.program_database.get_context_programs(2, parent_program.id)

            llm_suggestion_content = None
            refinement_feedback = None

            temp_codebase_for_prompting = copy.deepcopy(self.working_codebase)
            temp_codebase_for_prompting.get_block(target_block_name).update_code(parent_program.code_str)
            ...
            for attempt in range(self.run_config.self_refine_attempts + 1):
                ...
                try:
                    ...
                    logger.info(f"Self-Refinement: Code for parent {parent_program.id} is syntactically valid on attempt {attempt + 1}.")
                    llm_suggestion_content = generated_code
                    break
                except SyntaxError as e:
                    logger.warning(f"Self-Refinement: Attempt {attempt + 1} for parent {parent_program.id} failed syntax check. Error: {e}")
                    refinement_feedback = f"The code you provided has a syntax error: {e}. Please fix it."
                    temp_codebase_for_prompting.get_block(target_block_name).update_code(generated_code)

            if llm_suggestion_content is None:
                llm_suggestion_content = generated_code
                logger.warning(f"Self-Refinement failed after {self.run_config.self_refine_attempts + 1} attempts. Proceeding with last attempt.")
    ...
    def tell(self, evaluated_programs: List[Program]):
        for prog in evaluated_programs:
            if self.feature_extractor_fn and prog.features is None:
                prog.features = self.feature_extractor_fn(prog.eval_details, self.problem_specific_feature_configs, self.feature_definitions)
            self.program_database.add_program(prog)

            current_best_score = self.overall_best_solution.scores.get('main_score', -float('inf')) if self.overall_best_solution else -float('inf')
            if prog.scores.get('main_score', -float('inf')) > current_best_score:
                self.overall_best_solution = copy.deepcopy(prog)
                if prog.block_name:
                    self.working_codebase.update_block_code(prog.block_name, prog.code_str)

    def get_best_solution(self):
        return self.program_database.get_best_program()

    def get_emigrants(self, num_emigrants: int):
        all_progs = sorted(self.program_database.get_all_programs(), key=lambda p: p.scores.get('main_score', -float('inf')), reverse=True)
