...
def test_traditional_orchestrator(api_key: str):
    ...
    try:
        ...
        result = (agent
                 .define_problem(get_initial_code, simple_evaluator)
                 .add_stage("traditional_stage", max_generations=2, llm_settings=llm_settings, 
        ...
        if result.best_program:
            logger.info(f"Traditional Best Score: {result.best_program.scores.get('main_score', 'N/A')}")
...
def test_langgraph_backend(api_key: str):
    ...
    try:
        ...
        result = (agent
                 .define_problem(get_initial_code, simple_evaluator)
                 .add_stage("langgraph_stage", max_generations=2, llm_settings=llm_settings,
        ...
        if result.best_program:
            logger.info(f"LangGraph Best Score: {result.best_program.scores.get('main_score', 'N/A')}")
        
        return result
        
    except ImportError as e:
        if "Python 3.9+" in str(e):
            logger.warning("⚠️  LangGraph requires Python 3.9+ for type hint compatibility")
...
def compare_results(traditional_result, langgraph_result):
    ...
    if traditional_result and langgraph_result:
        ...
        trad_score = traditional_result.best_program.scores.get('main_score', -float('inf')) if traditional_result.best_program else -float('inf')
        lang_score = langgraph_result.best_program.scores.get('main_score', -float('inf')) if langgraph_result.best_program else -float('inf')
