...
class LLMManager:
    def __init__(self, default_api_key: str, llm_settings_list: Optional[List[LLMSettings]] = None,
                 selection_strategy: str = "weighted_random", **kwargs):
        self.default_api_key = default_api_key
        self.llm_instances: List[Dict[str, Any]] = []
        self.selection_strategy = selection_strategy
        self.current_llm_index = 0

        self.min_inter_request_delay_sec: float = float(kwargs.get('min_inter_request_delay_sec', 1.1))
        self.last_api_call_timestamp: float = 0.0
        logger.info(f"LLMManager: Minimum inter-request delay set to {self.min_inter_request_delay_sec:.2f}s.")
        ...
        if not llm_settings_list:
            logger.info("LLMManager: No LLM ensemble, using default 'gemini-1.5-flash-latest'.")
        ...
        logger.info(f"LLMManager ready with {len(self.llm_instances)} model(s). Strategy: '{self.selection_strategy}'.")
    ...
    def _select_llm(self) -> Dict[str, Any]:
        ...
        models, weights = [inst for inst in self.llm_instances], [inst['settings'].selection_weight for inst in self.llm_instances]
    ...
    def generate_code_modification(self, prompt: str, attempt_info: Optional[Dict[str, Any]] = None) -> Optional[str]:
        ...
        if time_since_last_call < self.min_inter_request_delay_sec:
            ...
            if sleep_duration > 0:
                logger.info(f"Delaying for {sleep_duration:.2f}s to respect inter-request delay.")
                time.sleep(sleep_duration)

        selected_llm = self._select_llm()
        settings = selected_llm['settings']

        logger.info(f"Sending prompt to Worker LLM ({settings.model_name}). Full prompt in DEBUG mode.")
        logger.debug(f"--- Full Prompt for {settings.model_name} ---\n{prompt}\n--- End of Prompt ---")
        ...
        try:
            ...
            if not response_text or not response_text.strip():
                logger.warning(f"LLM ({settings.model_name}) returned an empty response.")
                return None
            ...
            logger.warning(f"Could not extract structured code/diff from LLM response. Returning None.")
            logger.debug(f"--- Failed Raw Response ---\n{response_text}\n---")
            return None

        except Exception as e:
            self.last_api_call_timestamp = time.time()
            logger.error(f"LLM call failed for model {settings.model_name}: {e}", exc_info=True)
            return None
