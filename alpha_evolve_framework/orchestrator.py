...
class MainLoopOrchestrator:
    def __init__(self, optimizers: List[BaseOptimizer], evaluator: BaseEvaluator, run_config: RunConfiguration, initial_codebase: Codebase, llm_judge: Optional[LLMJudge] = None, task_description: str = "", stage_name: str = "DefaultStage", generation_logger: Optional[Callable] = None, judge_visual_generator: Optional[Callable] = None, judge_feedback_combiner: Optional[Callable] = None):
        ...
        logger.info(f"MainLoopOrchestrator initialized with {len(self.optimizers)} optimizer(s).")
        if self.run_config.use_island_model:
            logger.info(f"Island model enabled: {self.run_config.num_islands} islands, {self.run_config.island_generations_per_epoch} generations per epoch.")
        else:
            logger.info("Single optimizer mode.")

    def evolve(self, initial_population: Optional[List[Program]] = None, max_runtime_minutes: float = 60.0) -> StageOutput:
        ...
        try:
            ...
            while self.global_generation < self.run_config.max_generations:
                ...
                if elapsed_minutes > max_runtime_minutes:
                    logger.warning(f"Runtime limit ({max_runtime_minutes} minutes) exceeded. Stopping evolution.")
                    break
                ...
                if self.overall_best_program:
                    logger.info(f"Current best program: {self.overall_best_program.id} with score {self.overall_best_program.scores.get('main_score', -float('inf')):.4f}")
            ...
            logger.info(f"--- Orchestrator Stage '{self.stage_name}' Finished in {total_time:.2f}s ---")
            status = "COMPLETED"
            message = f"Stage '{self.stage_name}' completed successfully after {self.global_generation} generations."
            
        except Exception as e:
            logger.error(f"Orchestrator stage '{self.stage_name}' failed with an exception: {e}", exc_info=True)
            status = "FAILED"
        ...
        sorted_programs = sorted(unique_programs, key=lambda p: p.scores.get('main_score', -float('inf')), reverse=True)
        
        logger.info(f"Orchestrator returning {len(sorted_programs)} unique programs to the pipeline.")
    ...
    def _run_island_epoch(self):
        ...
        for optimizer in self.optimizers:
            ...
            while not optimizer.should_terminate():
                ...
                candidate_suggestions = optimizer.ask()
                if not candidate_suggestions:
                    logger.warning(f"Island {optimizer.island_id}: No candidates generated.")
                    optimizer.increment_generation()
                    continue
    ...
    def _evaluate_candidates(self, candidate_suggestions: List[Dict[str, Any]]) -> List[Program]:
        ...
        for candidate_info in candidate_suggestions:
            ...
            code_str = candidate_info['code_str']
            ...
            if self.llm_judge:
                judge_feedback = self.llm_judge.judge_program(program, self.task_description)
                if judge_feedback:
                    program.judge_feedback = judge_feedback
            
            evaluated_programs.append(program)
            self.all_evaluated_programs.append(program)
            self.evaluation_step_counter += 1
            
            logger.info(f"Evaluated {candidate_id}: Score = {scores.get('main_score', -float('inf')):.4f}")
    ...
    def _perform_migration(self):
        ...
        if self.run_config.migration_strategy == "ring":
            ...
            for i, optimizer in enumerate(self.optimizers):
                ...
                emigrants = optimizer.get_emigrants(self.run_config.migration_num_emigrants)
                if emigrants:
                    self.optimizers[next_island_idx].receive_immigrants(emigrants)
                    logger.info(f"Island {optimizer.island_id} -> Island {self.optimizers[next_island_idx].island_id}: {len(emigrants)} emigrants")
                    
        elif self.run_config.migration_strategy == "broadcast_best_to_all":
            # Find best optimizer and broadcast its best to all others
            best_optimizer = max(self.optimizers, key=lambda opt: opt.get_best_solution().scores.get('main_score', -float('inf')) if opt.get_best_solution() else -float('inf'))
            emigrants = best_optimizer.get_emigrants(self.run_config.migration_num_emigrants)
            
            if emigrants:
                for optimizer in self.optimizers:
                    if optimizer != best_optimizer:
                        optimizer.receive_immigrants(copy.deepcopy(emigrants))
                        logger.info(f"Island {best_optimizer.island_id} -> Island {optimizer.island_id}: {len(emigrants)} emigrants")

    def _update_overall_best(self):
        """Update the overall best program across all optimizers."""
        current_best = self.overall_best_program
        current_best_score = current_best.scores.get('main_score', -float('inf')) if current_best else -float('inf')
        
        for optimizer in self.optimizers:
            optimizer_best = optimizer.get_best_solution()
            if optimizer_best:
                optimizer_best_score = optimizer_best.scores.get('main_score', -float('inf'))
    ...
    def get_final_codebase(self) -> Codebase:
        ...
        if self.overall_best_program.block_name:
            final_codebase.update_block_code(self.overall_best_program.block_name, self.overall_best_program.code_str)
    ...
    def get_evolution_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'best_program_id': self.overall_best_program.id if self.overall_best_program else None,
            'best_score': self.overall_best_program.scores.get('main_score', -float('inf')) if self.overall_best_program else -float('inf'),
