...
class LLMJudge:
    ...
    def _build_judge_prompt(self, program: Program, task_description: str, image: Optional[Image.Image] = None) -> List[Any]:
        eval_details_json = json.dumps(program.eval_details, indent=2, default=numpy_converter)
    ...
    def judge_program(self, program: Program, task_description: str, image: Optional[Image.Image] = None) -> Optional[Dict[str, Any]]:
        ...
        logger.info(f"Judging Program {program.id} with model {self.settings.model_name}...")
        logger.debug(f"--- Full Judge Prompt for {program.id} ---\n{prompt_text}\n--- End of Judge Prompt ---")
        ...
        try:
            ...
            if not json_match:
                logger.error(f"Judge Error: No JSON object found in response for {program.id}.")
                return None

            feedback = json.loads(json_match.group(0))

            if 'critique' not in feedback or 'suggestions' not in feedback or 'judge_score' not in feedback:
                logger.error(f"Judge Error: JSON response for {program.id} is missing required keys.")
                return None

            logger.info(f"Judge for {program.id} Complete. Assigned Score: {feedback.get('judge_score')}")
            return feedback

        except Exception as e:
            logger.error(f"LLM Judge API call or parsing failed for program {program.id}: {e}", exc_info=True)
            return None
