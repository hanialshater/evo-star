...
class Program:
    ...
    def __repr__(self):
        ...
        judge_score_str = (
            f", judge_score={self.scores.get('judge_score', 'N/A')}"
            if "judge_score" in self.scores
            else ""
        )
        return (
            f"Program(id='{self.id}', block='{self.block_name}', "
...
class EvolveBlock:
    ...
    def apply_structured_diff(self, diff_str: str) -> bool:
        ...
        print(
            f"--- Received Diff String (stripped, first 500 chars): ---\n{diff_str.strip()[:500]}...\n---"
        )
        ...
        separator_marker_pos = normalized_diff_str.find(
            separator_marker_str, search_content_start_idx
        )
        if separator_marker_pos == -1:
            print(
                f"Diff Parse Error: '{separator_marker_str}' not found after '{search_marker_str}'."
            )
            return False
        search_code = normalized_diff_str[
            search_content_start_idx:separator_marker_pos
        ].strip()
        replace_content_start_idx = separator_marker_pos + len(separator_marker_str)
        replace_marker_actual_pos = normalized_diff_str.rfind(
            replace_marker_str, replace_content_start_idx
        )
        if (
            replace_marker_actual_pos == -1
            or replace_marker_actual_pos < replace_content_start_idx
        ):
            print(
                f"Diff Parse Error: '{replace_marker_str}' not found after '{separator_marker_str}'."
            )
            print(
                f"--- Section where REPLACE marker was expected: ---\n{normalized_diff_str[replace_content_start_idx:].strip()[:300]}...\n---"
            )
            return False
        replace_code = normalized_diff_str[
            replace_content_start_idx:replace_marker_actual_pos
        ].strip()
        ...
        if search_code not in normalized_current_code:
            print(
                f"Diff Apply Error: SEARCH code not found in current code of block '{self.name}'."
            )
            return False
        new_block_code_normalized = normalized_current_code.replace(
            search_code, replace_code, 1
        )
        if new_block_code_normalized == normalized_current_code:
            print(
                f"Diff Warning: No change after applying diff to block '{self.name}'."
            )
        self.current_code = new_block_code_normalized
        print(
            f"Diff applied successfully to block '{self.name}'. New code len: {len(self.current_code)}"
        )
        return True
