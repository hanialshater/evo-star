...
class SimpleProgramDatabase(BaseProgramDatabase):
    def __init__(self, population_size: int = 10):
        ...
        print(
            f"SimpleProgramDatabase initialized with population_size: {population_size}"
        )

    def add_program(self, program: Program):
        if program.id in self.programs:
            print(f"Warning: Program '{program.id}' exists. Overwriting.")
        self.programs[program.id] = program
        print(
            f"Added {program.id} (Score: {program.scores.get('main_score',-float('inf')):.4f}). Total: {len(self.programs)}"
        )
        self._prune()
    ...
    def select_parent_program(self, strategy: str = "best") -> Optional[Program]:
        ...
        if selected:
            print(f"Selected parent ({strategy}): {selected.id}")
        return selected
